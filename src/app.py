...
from models import db, User, Characters, Planets, Species, Starships, Favorites
...
@app.route('/user/<int:user_id>/favorites/species/<int:specie_id>', methods=['POST'])
def add_specie_favorite(user_id, specie_id):
    ...
    if Favorites.query.filter_by(user_id=user_id, specie_id=specie_id).first():
        raise APIException('The specie is already on the favorites list', status_code=400)
    favorite = Favorites(user_id=user_id, specie_id=specie_id)
    db.session.add(favorite)
    db.session.commit()
    return jsonify("Specie added to favorites successfully"), 200

@app.route('/user/<int:user_id>/favorites/species/<int:specie_id>', methods=['DELETE'])
...
@app.route('/speciess', methods=['GET'])
def get_speciess():
...
def specie(specie_id):
    ...
    return jsonify(specie_query.serialize()), 200
